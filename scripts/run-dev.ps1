...
📊 접속 정보:
   ...
   📋 DB 정보: devdb / devuser / devpass

💡 위의 '자동 로그인' URL을 브라우저 북마크로 저장하세요!
...
# Spring Boot 실행
Write-Host "🌱 Spring Boot 애플리케이션 시작 중..." -ForegroundColor Green
Write-Host "   🌐 웹 애플리케이션: http://localhost:17070" -ForegroundColor Blue
Write-Host ""

# Gradle wrapper 확인 및 실행
$gradleWrapper = if ($IsWindows) { ".\gradlew.bat" } else { "./gradlew" }

if (Test-Path $gradleWrapper) {
    try {
        & $gradleWrapper bootRun --args='--spring.profiles.active=dev'
    } catch {
        Write-Error "❌ Spring Boot 실행 실패: $_"
    }
} else {
    Write-Error "❌ Gradle wrapper를 찾을 수 없습니다. 'gradle wrapper' 명령을 실행해주세요."
}